...
bool RegionBuilder::runOnModule(Module &M) {
...
void RegionBuilder::mark_region(
	...
	forall(DenseSet<const ICFGNode *>, it, visited) {
		...
		assert(mbb);
		BasicBlock::iterator s = mbb->begin(), e = mbb->end();
		// <s_ins> is considered to be in this region, but <e_ins> is not. 
		if (mbb == MBBB.parent(s_ins))
			s = s_ins;
		if (mbb == MBBB.parent(e_ins))
			e = e_ins;
		for (BasicBlock::iterator i = s; i != e; ++i) {
			vector<Value *> ops;
			const IntegerType *int_type = IntegerType::get(i->getContext(), 32);
			const IntegerType *long_type = IntegerType::get(
					i->getContext(), __WORDSIZE);
			// Are you sure size_t == unsigned long? 
			ops.push_back(ConstantInt::get(int_type, thr_id));
			ops.push_back(ConstantInt::get(long_type, s_tr));
			ops.push_back(ConstantInt::get(long_type, e_tr));
			i->setMetadata(
					"region", MDNode::get(i->getContext(), &ops[0], ops.size()));
		}
	}
}
